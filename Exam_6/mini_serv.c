...
#include <netinet/ip.h> //  for struct sockaddr_in
...
void	notify_other(int owner, char *msg){
	for (int fd = 0; fd <= max_fd; fd++){
		if (FD_ISSET(fd, &wfds) && fd != owner)
...
int		create_socket(){
	...
	if (max_fd < 0)
		fatal_error(); // remember to change here from the main
	// END OF COPY-PASTE FROM MAIN
	FD_SET(max_fd, &afds);
	return max_fd;
}
	...
	while (1){
		...
		for (int fd = 0; fd <= max_fd; fd++){
			...
			if (fd == sockfd){ // new connection
				...
				if (read_bytes <= 0){ // client wants to disconnect, here is <= 0
					remove_client(fd);
					break ;
				}
